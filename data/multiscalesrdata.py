...
class SRData(data.Dataset):
    def __init__(self, args, name='', train=True, benchmark=False):
        ...
        if args.ext.find('img') < 0:
            path_bin = os.path.join(self.a_path, 'bin')
        ...
        if args.ext.find('bin') >= 0:
            ...
            if args.ext.find('img') >= 0 or benchmark:
                ...
                os.makedirs(
                    self.dir_hr.replace(self.a_path, path_bin),
                    exist_ok=True
                )
                if self.args.asymm:
                    for si, s in enumerate(self.scale_1):
                        s2 = self.scale_2[si]
                        if s2 == s:
                            os.makedirs(
                                os.path.join(
                                    self.dir_lr.replace(self.a_path, path_bin),
                                    'X{:.2f}'.format(s)
                                ),
                                exist_ok=True
                            )
                ...
                self.images_hr, self.images_lr = [], [[] for _ in range(len(self.scale_1))]
                for h in list_hr:
                    b = h.replace(self.a_path, path_bin)
                ...
                for i, ll in enumerate(list_lr):
                    for l_img in ll:
                        b = l_img.replace(self.a_path, path_bin)
                        b = b.replace(self.ext[1], '.pt')
                        self.images_lr[i].append(b)
                        self._check_and_load(
                            args.ext, [l_img], b, verbose=True, load=False
                        )
    ...
    def _scan(self):
        ...
        if self.args.asymm:
            names_lr = [[] for _ in range(len(self.scale_1))]
            for f in names_hr:
    ...
    def _set_filesystem(self, dir_data):
        self.a_path = os.path.join(dir_data, self.name)
    ...
    def get_patch(self, lr, hr):
        scale_1 = self.scale_1[self.idx_scale]
        scale_2 = self.scale_2[self.idx_scale]
        if self.train:
            if self.args.asymm:
            ...
            ih, iw = lr.shape[:2]
            hr = hr[0:int(ih * scale_1), 0:int(iw * scale_2)]
